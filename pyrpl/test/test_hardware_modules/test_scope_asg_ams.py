...
class TestScopeAsgAms(TestPyrpl):
    ...
    def test_asg(self):
        ...
        for asg in [self.r.asg0, self.r.asg1]:
            ...
            expect = (
                ...
                / 8191
                * np.round(
                    8191.0 * np.sin(np.linspace(0, 2 * np.pi, asg.data_length, endpoint=False))
                )
            )
    ...
    def test_asg_to_scope(self):
        ...
        for asg in [self.r.asg0, self.r.asg1]:
            ...
            asg.setup(
                waveform="ramp",
                frequency=1.0 / self.r.scope.duration,
            ...
            expect *= -1
            self.r.scope.input1 = (
                asg  # Bijection(self.r.scope._ch1._inputs).inverse[asg._dsp._number]
            )
            self.r.scope.input2 = asg.name
            # asg.trig()
            self.r.scope.setup(
                trigger_source=self.r.scope.input1, trace_average=1
            )  # the asg trigger
