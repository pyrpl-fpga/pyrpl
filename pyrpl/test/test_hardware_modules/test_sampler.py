...
class TestInput(TestPyrpl):
    ...
    def test_sampler(self):
        with self.pyrpl.asgs.pop("test_sampler") as asg:
            ...
            assert max <= asg.offset + asg.amplitude, (
                mean,
                std,
                max,
                min,
                asg.offset + asg.amplitude,
            )
            # needs a small margin to work properly because
            # of rounding off towards negative values in asg
