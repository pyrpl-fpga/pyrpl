...
class TestAttributeClass(TestPyrpl):
    ...
    def test_dummy_module(self):
        ...
        assert isinstance(
            self.pyrpl.dummymodule.some_filter, numbers.Number
    ...
    def test_submodule(self):
        ...
        assert self.sub1.b1  # values defined in config file
        assert not self.sub1.b2

        self.sub2.b1 = True
        self.sub2.b2 = False
        assert self.sub2.b1
        assert not self.sub2.b2

        self.pyrpl.dummymodule.save_state("true_false_true_false")

        self.sub1.b1 = False
        self.sub1.b2 = True
        assert not self.sub1.b1
        assert self.sub1.b2

        self.sub2.b1 = False
        self.sub2.b2 = True
        assert not self.sub2.b1
        assert self.sub2.b2
