...
# returns the expected signal value as a function of the variable value. This function ensures that
# the correct setpoint and a reasonable gain is chosen (from the derivative of expected_signal)
# when this signal is used for feedback.
class CustomInputClass(InputSignal):
    """A custom input signal for our customized lockbox. Please refer to the documentation on the
    default API of InputSignals"""

    def expected_signal(self, variable):
        # For example, assume that our analog signal is proportional to the square of the variable
        return (
            self.calibration_data.min
            + self.custom_gain_attribute * self.lockbox.custom_attribute * variable**2
        )

    # If possible, you should explicitely define the derivative of expected_signal(variable).
    # Otherwise, the derivative is estimated numerically which might lead to inaccuracies and
    # excess delay.
    ...
    custom_gain_attribute = FloatProperty(
        default=1.0,
        min=-1e10,
        max=1e10,
        increment=0.01,
    ...
    # A customized calibration method can be used to implement custom calibrations. The calibration
    # method of the InputSignal class retrieves min, max, mean, rms of the input signal during a
    # sweep and saves them as class attributes, such that they can be used by expected_signal().
    def calibrate(self):
        """This is a simplified calibration method. InputSignal.calibrate works better than this in
        most cases."""
        self.lockbox.sweep()
...
class CustomLockbox(Lockbox):
    ...
    # this syntax for the definition of inputs and outputs allows to conveniently access inputs in
    # the API
    inputs = LockboxModuleDictProperty(
        custom_input_name1=CustomInputClass, custom_input_name2=CustomInputClass
    )

    outputs = LockboxModuleDictProperty(
        slow_output=OutputSignal, fast_output=OutputSignal, pwm_output=OutputSignal
    )

    # the name of the variable to be stabilized to a setpoint. inputs.expected_signal(variable)
    # returns the expected signal as a function of this variable
    variable = "displacement"

    # attributes are defined by descriptors
    custom_attribute = FloatProperty(default=1.0, increment=0.01, min=1e-5, max=1e5)

    # list of attributes that are mandatory to define lockbox state. setup_attributes of all base
    # classes and of all submodules are automatically added to the list by the metaclass of Module
    ...
    # if nonstandard units are to be used to specify the gain of the outputs, their conversion to
    # Volts must be defined by a property called _unitname_per_V
    _mV_per_V = 1000.0
    _units = ["V", "mV"]
...
class ExampleLoop(LockboxPlotLoop):  # or inherit from
    ...
    def loop(self):
        ...
        self.plot.append(
            green=np.sin(2.0 * np.pi * tact * 3),  # /self.lockbox.interval),
            red=dt,
        )
        if self.c.n == 100:
            self.result = 42
...
class GalvanicIsolationLoopLockbox(Lockbox):
    ...
    def start_gi(self):
        ...
        # start loop
        self.galvanic_isolation_loop = LockboxLoop(
            parent=self,
    ...
    def galvanic_isolation_loop_function(self):
        ...
        # only for debugging:
        # self.second_pyrpl.rp.asg0.offset = np.sin(2*np.pi*time()
        #           - self.galvanic_isolation_loop.loop_start_time)
...
class ShortLoopLockbox(Lockbox):
    ...
    def plot_sin_and_in1(lockbox_self, loop_self):
        ...
        loop_self.plot.append(
            green=np.sin(2 * np.pi * loop_self.time),
    ...
    loop = ModuleProperty(
        LockboxPlotLoop, interval=0.05, autostart=True, loop_function=plot_sin_and_in1
    )
