...
class CalibrationData(LockboxModule):
    ...
    _asg_phase = PhaseProperty(
        doc="Phase of the asg when error signal is centered "
    ...
    def get_stats_from_curve(self, curve):
        ...
        if curve is None:
            self.logger.warning(
                "Curve object for calibration is None. No calibration will be performed."
            )
        else:
...
class Signal(LockboxModule, SignalModule):
    ...
    def signal(self):
        ...
        raise ValueError(
            "Please define the method 'signal()' if the Signal "
    ...
    def get_analog_offset(self, duration=1.0):
        ...
        self._logger.info(
            "Calibrated analog offset of signal %s. "
    ...
    def stats(self, t=None):
        ...
        if (
            not hasattr(self, "_lasttime")
            or t is not None
            or time() - self._lasttime >= self.sampler_time
        ):
            # choose sampler time
            if t is None:
                t = self.sampler_time
...
class InputSignal(Signal):
    ...
    *unit*. The signal can be calibrated by taking a curve while scanning
    ...
    input_signal = InputSelectProperty(
        call_setup=True, doc="the dsp module or lockbox signal used as input signal"
    )
    ...
    def _input_signal_dsp_module(self):
        ...
        self._logger.warning(
            "Input signal of input %s cannot be traced "
    ...
    def signal(self):
        """returns the signal corresponding to this module that can be used to connect the signal
        to other modules. By default, this is the direct input signal."""
    ...
    def expected_signal(self, variable):
        ...
        raise NotImplementedError(
            "Formula relating variable and parameters to output should be implemented in derived "
            "class"
        )
    ...
    def is_locked(self, loglevel=logging.INFO):
        ...
        self._logger.log(
            loglevel,
            "Locked at stage %s: input %s value is %.2f +- %.2f (setpoint %.2f).",
            self.lockbox.current_stage.name,
            self.name,
            actmean,
            actrms,
...
class InputFromOutput(InputDirect):
    ...
    def is_locked(self, loglevel=logging.INFO):
        ...
        if forwarded_input is not None:
            self._logger.debug(
                "is_locked() for InputFromOutput '%s' is "
                "forwarded to is_locked() of input signal '%s'.",
                self.name,
                forwarded_input.name,
            )
            return forwarded_input.is_locked(loglevel=loglevel)
        else:
            self._logger.warning(
                "is_locked() for InputFromOutput '%s' is not "
