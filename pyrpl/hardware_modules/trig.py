...
class Trig(FilterModule):
    ...
    _trigger_sources = {"off": 0, "pos_edge": 1, "neg_edge": 2, "both_edge": 3}
    trigger_sources = sorted(_trigger_sources.keys())  # help for the user
    trigger_source = SelectRegister(
        0x108, doc="Trigger source", options=_trigger_sources, default="off"
    )

    _output_signals = sorted_dict(TTL=0, asg0_phase=1)
    output_signals = _output_signals.keys()
    output_signal = SelectRegister(
        0x10C, options=_output_signals, doc="Signal to use as module output"
    )
    phase_offset = PhaseRegister(
        0x110,
        bits=14,
        doc="offset to add to the output phase (before taking absolute value)",
    )

    threshold = FloatRegister(0x118, bits=14, norm=2**13, doc="trigger threshold [volts]")

    hysteresis = FloatRegister(0x11C, bits=14, norm=2**13, doc="hysteresis for ch1 trigger [volts]")
    ...
    def output_signal_to_phase(self, v):
        ...
        :math:`0\\,\\mathrm{V} = 0\\deg,\\, -1\\,\\mathrm{V} = 180\\deg,\\,`
        :math:`1\\,\\mathrm{V} = 180\\deg - \\epsilon\\,.`
