...
class DspModule(HardwareModule, SignalModule):
    ...
    def inputs(self):
        self._logger.warning(
            "Deprecation warning: DspModule.inputs "
            "will soon be removed. Use "
            "DspModule.input_options instead!"
        )
        return all_inputs(self).keys()

    input = InputSelectRegister(
        0x0, options=all_inputs, doc="selects the input signal of the module"
    )

    @property
    def output_directs(self):
        self._logger.warning(
            "Deprecation warning: DspModule.output_directs"
            "will soon be removed. Use "
    ...
    output_direct = SelectRegister(
        ...
        doc="selects to which analog output the module signal is sent directly",
    )
        ...
        if not modules:
            ...
            for module in modules:
                if isinstance(module, DspModule):
                    ...
                    self._logger.warning(
                        "Module specified by %s has unknown "
    ...
    current_output_signal = FloatRegister(
        ...
        bits=14,
        norm=2**13 - 1,
        doc="current value of output_signal as returned by the sampler module",
    )
