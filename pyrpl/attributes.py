...
class BaseProperty(BaseAttribute):
    ...
    * a function get_value(instance) that reads the value from
    ...
    def __get__(self, instance, owner):
        ...
        # and potentially many obj instances
        # (think of having 2 redpitayas in the same python session), then
...
class BaseRegister(BaseProperty):
    ...
    Interface for basic register of type int. To convert the value between register format and
    python readable format, registers need to implement "from_python" and "to_python" functions"""
    ...
    def _reads(self, obj, addr, length):
        return obj._reads(addr, length)
...
class LongRegister(IntRegister):
    """Interface for register of python type int/long with arbitrary length 'bits'
...
class FloatRegister(IntRegister, FloatProperty):
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if not self.signed:
            value = abs(value)
        return FloatProperty.validate_and_normalize(
            self, obj, round(value / self.increment) * self.increment
        )
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if rounded_value == 0 and value != 0:  # value was rounded off to zero
            if self.avoid_round_off_to_zero:
                ...
                obj._logger.warning(
                    "Avoided rounding value %.1e of the "
        ...
        if value > self.max or value < self.min:
            obj._logger.warning(
                "Requested gain for %s.%s is outside the "
...
class FrequencyRegister(FloatRegister, FrequencyProperty):
    ...
    def to_python(self, obj, value):
        return 125e6 / 2**self.bits * float(value) * obj._frequency_correction

    def validate_and_normalize(self, obj, value):
        """
        Same as FloatRegister, except the value should be positive.
        """
        return FrequencyProperty.validate_and_normalize(
            self, obj, FloatRegister.validate_and_normalize(self, obj, value)
        )
...
class FilterProperty(BaseProperty):
    ...
    self.valid_frequencies(module) (evaluated at runtime). If floats are provided, they are
    normalized to the nearest values in the list. Individual floats are also normalized to a
    singleton.
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if not np.iterable(value):
            value = [value]
        value = [
            min([opt for opt in self.valid_frequencies(obj)], key=lambda x: abs(x - val))
            for val in value
        ]
        if len(value) == 1:
            return value[0]
    ...
    def valid_frequencies(self, module):
        raise NotImplementedError(
            "this is a baseclass, your derived class must implement the following function"
        )
...
class FilterRegister(BaseRegister, FilterProperty):
    ...
    def valid_frequencies(self, obj):
        ...
        if self._FILTERSTAGES(obj) == 0:
            # A bitstream can deliberately omit a module's input filters.
            # In that case zero (bypass) is the only valid setting. Calling
            # to_python() would return an empty list, which cannot provide a
            # representative cutoff frequency.
            valid_frequencies = [0]
        else:
            # valid_bits = range(0, self._MAXSHIFT(obj)-1)  # this is possible
            valid_bits = range(
                0, self._MAXSHIFT(obj) - 2
            )  # this gives reasonable results (test_filter)
        ...
        if (
            obj is not None
            and not hasattr(obj.__class__, self.name + "_options")
            and not hasattr(obj, self.name + "_options")
        ):
    ...
    def to_python(self, obj, value):
        ...
        negative bandwidth stands for high-pass instead of lowpass, 0 bandwidth for bypassing the
        filter
        """
        filter_shifts = value
        bandwidths = []
...
class PWMRegister(FloatRegister):
    ...
    def __init__(self, address, CFG_BITS=24, PWM_BITS=8, **kwargs):
        ...
        self.max = 1.8
        self.increment = (self.max - self.min) / 2 ** (
            self.bits - 1
        )  # actual resolution is 14 bits (roughly 0.1 mV incr.)
...
class SelectProperty(BaseProperty):
    ...
    def options(self, instance=None):
        ...
        options may be callable with instance as optional argument.
        """
        ...
        if instance is not None:
            if len(options) == 0:
                logger.debug(
                    "SelectProperty %s of module %s has no options!",
                    self.name,
                    instance,
                )
                options = {None: None}
...
class SelectRegister(BaseRegister, SelectProperty):
    ...
    def get_value(self, obj):
        ...
        if raw_value != expected_value:
            obj._logger.warning(
                "Register %s of module %s has value %s, "
...
class ProxyProperty(BaseProperty):
    ...
    def change_options(self, obj, new_options):
        if obj is None:
            obj = self.instance
        module = recursive_getattr(obj, self.path_to_target_module)
        return recursive_getattr(obj, self.path_to_target_descriptor + ".change_options")(
            module, new_options
        )
            ...
            def forward_update_attribute_by_name(name, value):
                ...
                if name == self.target_attribute:
                    instance._signal_launcher.update_attribute_by_name.emit(
                        self.name, [self._target_to_proxy(instance, value[0])]
                    )
                    if self.call_setup:
                        instance.setup()
    ...
    def _create_widget(self, module, widget_name=None, **kwargs):
        ...
        #                                            **kwargs)
        self._widget_class = recursive_getattr(
            module, self.path_to_target_descriptor + "._widget_class"
        )
