...
class AnalogTfDialog(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        {"flat": self.flat, "filter": self.filter, "curve": self.curve}[
            self.module.tf_type
        ].setChecked(True)
...
class LockboxInputWidget(ModuleWidget):
    ...
    def show_lock(self, stage):
        ...
        self.curve_slope.setData(
            [setpoint - dx, setpoint + dx], [signal - slope * dx, signal + slope * dx]
        )
    ...
    def input_calibrated(self, input=None):
        ...
        input._logger.debug(
            "Updated widget for input %s to show GUI display of expected signal (min at %f)!",
            input.name,
            input.expected_signal(0),
        )
...
class AllSignalsWidget(QtWidgets.QTabWidget):
    ...
    def __init__(self, lockbox_widget):
        ...
        )  # otherwise + tab could be selected by
...
class LockboxStageWidget(ReducedModuleWidget):
    ...
    def create_title_bar(self):
        ...
        self.add_button.clicked.connect(
            lambda: self.module.parent.insert(self.module.name, self.module.setup_attributes)
        )
...
class LockboxSequenceWidget(ModuleWidget):
    ...
    def init_gui(self):
        ...
        self.button_add.clicked.connect(
            lambda: self.module.append(self.module[-1].setup_attributes)
        )
...
class LockboxWidget(ModuleWidget):
    """
    The LockboxWidget combines the lockbox submodules widget: model, inputs, outputs,
    lockbox_control
    """
    ...
    def init_gui(self):
        ...
        self.button_is_locked.clicked.connect(
            lambda: self.module.is_locked(loglevel=self.module._logger.getEffectiveLevel())
        )
        ...
        minimumsizehint = (
            self.sequence_widget.minimumSizeHint().height()
    ...
    def state_changed(self, statelist):
        ...
        if stage == "unlock":
            ...
            if stage != self.module.sequence[-1]:
                ...
                self._set_button_green(
                    self.module.sequence[-1]._widget.button_goto, color="darkGreen"
                )
            for (
                input,
                widget,
            ) in self.all_sig_widget.inputs_widget.input_widgets.items():
